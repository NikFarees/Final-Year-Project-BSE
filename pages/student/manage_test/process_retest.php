...
$requiredFields = ['test_id', 'student_license_id', 'test_fee', 'payment_option'];
...
$student_sql = "SELECT s.student_id FROM students s WHERE s.user_id = ?";
...
if ($student_result->num_rows === 0) {
    echo "<div class='alert alert-danger text-center'>Student record not found. 
          <a href='list_test.php' class='btn btn-primary btn-round mt-3'>Back to Test List</a></div>";
    exit;
}

// Verify test exists
$test_sql = "SELECT test_name FROM tests WHERE test_id = ?";
...
if ($test_result->num_rows === 0) {
    echo "<div class='alert alert-danger text-center'>Test not found. 
          <a href='list_test.php' class='btn btn-primary btn-round mt-3'>Back to Test List</a></div>";
    exit;
}
...
try {
    ...
    // Insert into student_tests with Pending status
    $insert_test_sql = "INSERT INTO student_tests (student_test_id, student_license_id, test_id, status, schedule_status) 
                        VALUES (?, ?, ?, 'Pending', 'Unassigned')";
    $insert_test_stmt = $conn->prepare($insert_test_sql);
    $insert_test_stmt->bind_param("sss", $student_test_id, $student_license_id, $test_id);
    ...
    // Insert payment record with type 'Retest'
    $payment_type = 'Retest';
    ...
    $insert_payment_stmt->bind_param("ssssds", $payment_id, $student_license_id, $payment_type, $payment_option, $test_fee, $payment_status);
    $insert_payment_stmt->execute();

    // Generate and insert payment detail
    ...
    $insert_payment_detail_stmt->bind_param("sssd", $payment_detail_id, $payment_id, $test_id, $test_fee);
    $insert_payment_detail_stmt->execute();

    // Commit transaction
    $conn->commit();
    
    // Store payment ID in session for receipt page
    $_SESSION['last_payment_id'] = $payment_id;
    
    // Show processing page with auto-redirect
?>
    <div class="container">
                        ...
                        <div class="card shadow-lg">
                            ...
                            <div class="card-body text-center">
                                <div class="my-5">
                                    ...
                                    <p class="mt-3">Your retest booking is being processed...</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        setTimeout(function() {
            window.location.href = 'retest_receipt.php?payment_id=<?php echo $payment_id; ?>';
        }, 2000);
    </script>
...
<?php
    ...
    echo "<div class='container'><div class='page-inner'><div class='page-category'>
          <div class='alert alert-danger text-center'>
            ...
            <a href='list_test.php' class='btn btn-primary mt-3'>Return to Test List</a>
          </div>
          </div></div></div>";
}
